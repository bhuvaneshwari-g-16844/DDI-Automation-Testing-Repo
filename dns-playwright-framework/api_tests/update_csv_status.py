...
with open(RESULTS_FILE) as f:
    for line in f:
        ...
        #   api_tests/folder/test_xxx.py::Class::func PASSED [nn%]
        m = re.search(r'api_tests/(\w+)/test_\w+\.py::(\w+)::(\w+)\s+(PASSED|FAILED|SKIPPED)', line)
        if m:
        ...
        #   api_tests/test_all_negative_boundary.py::Class::func[PARAM] PASSED [nn%]
        nb = re.search(r'api_tests/test_all_negative_boundary\.py::(\w+)::(\w+?)(?:\[([^\]]+)\])?\s+(PASSED|FAILED|SKIPPED)', line)
        if nb:
            cls, func, param, status = nb.groups()
            neg_results[(cls, func, param)] = status

        # Parse FAILED summary lines for reason
        fm = re.match(r'^FAILED\s+api_tests/(\w+)/test_\w+\.py::\w+::(\w+)\s+-\s+(.*)', line)
        if fm:
